...
var challenge4InputData []string = []string{
	"iyr:2013 hcl:#ceb3a1",
	"hgt:151cm eyr:2030",
	"byr:1943 ecl:grn",
	"",
...
var challenge4TestData []string = []string{
	"ecl:gry pid:860033327 eyr:2020 hcl:#fffffd",
	"byr:1937 iyr:2017 cid:147 hgt:183cm",
	"",
	"iyr:2013 ecl:amb cid:350 eyr:2023 pid:028048884",
...
var challenge4TestDataPart2 []string = []string{
	"eyr:1972 cid:100",
	"hcl:#18171d ecl:amb hgt:170 pid:186cm iyr:2018 byr:1926",
	"",
	"iyr:2019",
...
func Challenge4(useTestData bool) {
	var dataPart1 []string
	var dataPart2 []string
	if useTestData {
		dataPart1 = challenge4TestData
		dataPart2 = challenge4TestDataPart2
	} else {
		dataPart1 = challenge4InputData
		dataPart2 = challenge4InputData
	}

	println("Part 1")
	challenge4Part1(dataPart1)
	println("Part 2")
	challenge4Part2(dataPart2)
}

func challenge4Part1(data []string) {
	valid := 0
	start := 0

	for i := 0; i <= len(data); i++ {
		if i == len(data) || data[i] == "" {
			passportParsed := challenge4ParseData(data, start, i)
			start = i + 1
			_, byrCheck := passportParsed["byr"]
			_, iyrCheck := passportParsed["iyr"]
			_, eyrCheck := passportParsed["eyr"]
			_, hgtCheck := passportParsed["hgt"]
			_, hclCheck := passportParsed["hcl"]
			_, eclCheck := passportParsed["ecl"]
			_, pidCheck := passportParsed["pid"]
			if byrCheck && iyrCheck && eyrCheck && hgtCheck &&
				hclCheck && eclCheck && pidCheck {
				valid++
			}
		}
	}
...
func challenge4Part2(data []string) {
	valid := 0
	start := 0
	...
	for i := 0; i <= len(data); i++ {
		if i == len(data) || data[i] == "" {
			passportParsed := challenge4ParseData(data, start, i)
			start = i + 1
			if byr, exists := passportParsed["byr"]; exists {
			...
			if iyr, exists := passportParsed["iyr"]; exists {
			...
			if eyr, exists := passportParsed["eyr"]; exists {
			...
			if hgt, exists := passportParsed["hgt"]; exists {
				if len(hgt) < 2 {
					continue
				}
			...
			if hcl, exists := passportParsed["hcl"]; exists {
				if len(hcl) != 7 || hcl[0] != '#' {
					continue
				}
			...
			if ecl, exists := passportParsed["ecl"]; exists {
			...
			if pid, exists := passportParsed["pid"]; exists {
				if len(pid) != 9 {
					continue
				}
...
func challenge4ParseData(input []string, start, end int) map[string]string {
	completeStr := input[start]
	for i := start + 1; i < end; i++ {
		completeStr += " " + input[i]
	}

	splitBySpace := strings.Split(completeStr, " ")
	result := make(map[string]string)
	for _, prop := range splitBySpace {
