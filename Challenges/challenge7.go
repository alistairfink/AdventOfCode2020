...
So, a single shiny gold bag must contain 1 dark olive bag (and the 7 bags within it) plus 2 vibrant plum bags (and the 11 bags within each of
	those): 1 + 1*7 + 2 + 2*11 = 32 bags!

Of course, the actual rules have a small chance of going several levels deeper than this example; be sure to count all of the bags, even if
...
func Challenge7(useTestData bool) {
	var data []string
	var dataPart2 []string
	if useTestData {
		data = challenge7TestData
		dataPart2 = challenge7TestDataPart2
	} else {
		data = challenge7InputData
		dataPart2 = challenge7InputData
	}

	println("Part 1")
	challenge7Part1(data)
	println("Part 2")
	challenge7Part2(dataPart2)
}

func challenge7Part1(data []string) {
	bagsContain := make(map[string]bool)
	rabbitHole := make(map[string][]challenge7Data)
	for _, row := range data {
		outer, inner := parseChallenge7Data(row)
		rabbitHole[outer] = inner
	}

	for outerBag, _ := range rabbitHole {
		challenge7ItteratePart1(bagsContain, rabbitHole, "shiny gold", outerBag)
	}

	println(len(bagsContain))
}

func challenge7Part2(data []string) {
	rabbitHole := make(map[string][]challenge7Data)
	for _, row := range data {
		outer, inner := parseChallenge7Data(row)
		rabbitHole[outer] = inner
	...
	println(challenge7ItteratePart2(rabbitHole, "shiny gold"))
}

type challenge7Data struct {
	bag      string
	quantity int
}

func parseChallenge7Data(input string) (string, []challenge7Data) {
	cStart := 0
	outerBag := ""
	for i := 0; i < len(input); i++ {
		if input[i:i+4] == "bags" {
			outerBag = input[:i-1]
		} else if input[i:i+7] == "contain" {
			cStart = i
			break
		}
	}

	if input[cStart:] == "contain no other bags." {
		return outerBag, []challenge7Data{}
	}

	innerBags := input[cStart+8 : len(input)-1]
	innerBagsSplit := strings.Split(innerBags, ", ")
	result := []challenge7Data{}
	for _, bag := range innerBagsSplit {
		bagRes := challenge7Data{}
		quantityEnd := -1
		for i := 0; i < len(bag); i++ {
			if bag[i] == ' ' {
				quantityEnd = i
				bagRes.quantity, _ = strconv.Atoi(bag[:i])
				break
			}
		}
		if bag[len(bag)-3:] == "bag" {
			bagRes.bag = bag[quantityEnd+1 : len(bag)-4]
		} else if bag[len(bag)-4:] == "bags" {
			bagRes.bag = bag[quantityEnd+1 : len(bag)-5]
		}

		result = append(result, bagRes)
	}

	return outerBag, result
}

func challenge7ItteratePart1(bagsContain map[string]bool, rabbitHole map[string][]challenge7Data, target string, outer string) bool {
	for _, bag := range rabbitHole[outer] {
		if bag.bag == target || challenge7ItteratePart1(bagsContain, rabbitHole, target, bag.bag) {
			bagsContain[outer] = true
			return true
		}
	}
...
func challenge7ItteratePart2(rabbitHole map[string][]challenge7Data, outer string) int {
	result := 0
	for _, bag := range rabbitHole[outer] {
		result += bag.quantity
		result += bag.quantity * challenge7ItteratePart2(rabbitHole, bag.bag)
	}

	return result
}

var challenge7InputData []string = []string{
...
var challenge7TestData []string = []string{
	"light red bags contain 1 bright white bag, 2 muted yellow bags.",
	"dark orange bags contain 3 bright white bags, 4 muted yellow bags.",
	"bright white bags contain 1 shiny gold bag.",
	"muted yellow bags contain 2 shiny gold bags, 9 faded blue bags.",
...
var challenge7TestDataPart2 []string = []string{
	"shiny gold bags contain 2 dark red bags.",
	"dark red bags contain 2 dark orange bags.",
	"dark orange bags contain 2 dark yellow bags.",
	"dark yellow bags contain 2 dark green bags.",
