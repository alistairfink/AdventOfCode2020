...
However, the person sitting next to you seems to be experiencing a language barrier and asks if you can help. For each of the people in their group, you
...
Another group asks for your help, then another, and eventually you've collected answers from every group on the plane (your puzzle input). Each group's
answers are separated by a blank line, and within each group, each person's answers are on a single line. For example:
...
func Challenge6(useTestData bool) {
	var data []string
	if useTestData {
		data = challenge6TestData
	} else {
		data = challenge6InputData
	}

	println("Part 1")
	challenge6Part1(data)
	println("Part 2")
	challenge6Part2(data)
}

func challenge6Part1(data []string) {
	count := 0
	curr := ""
...
func challenge6Part2(data []string) {
	count := 0
	curr := ""
	currCount := 0
	for i := 0; i <= len(data); i++ {
...
var challenge6InputData []string = []string{
	"psyjxulrdtfe", "jeusdrlxyftp", "ufdpjsxrlztye", "yeorabxsdnhftujlp", "pedfxtsryujl", "",
...
var challenge6TestData []string = []string{
	"abc",
	"",
	"a",
	"b",
