...
func Challenge10(useTestData bool) {
	var data []int
	var part1Answer int
	if useTestData {
		data = challenge10TestData1
		part1Answer = 52
		// data = challenge10TestData2
		// part1Answer = 22
	} else {
		data = challenge10InputData
		part1Answer = 171
	}

	println("Part 1")
	challenge10Part1(data)
	println("Part 2")
	challenge10Part2(data, part1Answer)
}

func challenge10Part1(data []int) {
	used := make([]bool, len(data))
	sort.Ints(data)
	challenge10Part1Itterate(data, used, 0, []int{})
}

func challenge10Part2(data []int, part1Answer int) {
	sort.Ints(data)
	count := make([]int, len(data))
	for i := 0; i < len(data) && data[i] <= 3; i++ {
		count[i] = 1
	}

	for i := 0; i < len(data); i++ {
		for j := i - 1; j >= 0 && data[i]-data[j] <= 3; j-- {
			count[i] += count[j]
		}
	}

	println(count[len(count)-1])
}

func challenge10Part1Itterate(data []int, used []bool, prevUsed int, order []int) bool {
	if len(order) == len(used) {
		oneCount, threeCount := 0, 0
		if order[0] == 1 {
			oneCount++
		} else if order[0] == 3 {
			threeCount++
	...
	for i := 0; i < len(data); i++ {
		diff := data[i] - prevUsed
		if !used[i] && diff <= 3 && diff >= 0 {
			used[i] = true
			if challenge10Part1Itterate(data, used, data[i], append(order, data[i])) {
				return true
			}
...
var challenge10InputData []int = []int{
	79,
	142,
	139,
	33,
...
var challenge10TestData1 []int = []int{
	28,
	33,
	18,
	42,
...
var challenge10TestData2 []int = []int{
	16,
	10,
	15,
	5,
